...
@if(Auth::user())
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                Business School Submitted Accreditation Invoices.
                <small></small>
            </h1>
            <ol class="breadcrumb">
...
{{--                    </button>--}}
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span data-toggle="tooltip" title="Invoice Slip Details" >
                                                ...
                                                data-row='{"id":"{{$invoice->id}}","user":"{{$invoice->campus->user->name}}","designation":"{{$invoice->designation}}","school":"{{$invoice->campus->business_school->name}}","campus":"{{$invoice->campus->location}}","department_id":"{{$invoice->department->name}}","slip":"{{$invoice->slip}}","payment_method_id":"{{$invoice->payment_method_id}}","status":"{{$invoice->status}}","cheque_no":"{{$invoice->cheque_no}}","comments":"{{str_replace(array("\r\n", "\r", "\n"), "", $invoice->comments)}}","transaction_date":"{{$invoice->transaction_date}}","invoice_no":"{{$invoice->invoice_no}}","email":"{{$invoice->email}}"}' ></i> </span></td>
                                    </tr>
                                @endforeach
    ...
    <script type="text/javascript">
        ...
        $('#update-button').on('click', function () {
            ...
            $.ajax({
                url:'{{url("accreditation-invoice-Status")}}',
                type:'POST',
