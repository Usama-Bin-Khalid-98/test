@section('pageTitle', 'Extra Activities')
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active"> Extra Activities </li>
            </ol>
        </section>
                ...
                <div class="col-md-12">
                    ...
                    <div class="box box-primary">
                        <div class="box-header">
                            <h3 class="box-title">3.11. Provide data on extra-curricular and co-curricular activities conducted during last year in the Table 3.8.</h3>
                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <table id="datatable" class="table table-bordered table-striped">
                                ...
                                <tbody>
                               @foreach($extras as $plan)
                                <tr>
                                    <td>{{$plan->date}}</td>
                                    <td>{{$plan->activity_title}}</td>
                                    <td>{{$plan->budget_allocation}}</td>
                                    <td><i class="badge {{$plan->status == 'active'?'bg-green':'bg-red'}}">{{$plan->status == 'active'?'Active':'Inactive'}}</i></td>
                               <td><i class="fa fa-trash text-info delete" data-id="{{$plan->id}}"></i> | <i class="fa fa-pencil text-blue edit" data-row='{"id":"{{$plan->id}}", "date":"{{$plan->date}}", "activity_title":"{{$plan->activity_title}}", "budget_allocation":"{{$plan->budget_allocation}}", "status":"{{$plan->status}}"}' data-toggle="modal" data-target="#edit-modal"></i></td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Edit Extra Activities. </h4>
                </div>
    ...
    <script type="text/javascript">
      ...
      autoclose:true
         ...
         $('#form').submit(function (e) {
            ...
            let budget_allocation = $('#budget_allocation').val();

            !date?addClass('date'):removeClass('date');
            !activity_title?addClass('activity_title'):removeClass('activity_title');
            !budget_allocation?addClass('budget_allocation'):removeClass('budget_allocation');

            if(!date || !activity_title || !budget_allocation)
            {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(next){
                        setTimeout(() => {
                            window.location = '/alumni-membership';
                        }, 1000);
                    }else{
                        location.reload();
                    }
         ...
         $('.edit').on('click', function () {
            ...
            $('#edit_budget_allocation').val(data.budget_allocation);
            $('#edit_id').val(data.id);
...
$('#updateForm').submit(function (e) {
            ...
            let budget_allocation = $('#edit_budget_allocation').val();
            let id = $('#edit_id').val();
            ...
            !budget_allocation?addClass('edit_budget_allocation'):removeClass('edit_budget_allocation');
